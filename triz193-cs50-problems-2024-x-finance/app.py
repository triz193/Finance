...
def index():
    ...
    # Get user's stocks, shares and balance
    stocks = db.execute(
    ...
    # Initializing variables for total values
    total_value = cash
    grand_total = cash

    # Iterating over stocks and adding price and total value:
    for stock in stocks:
...
def register():
    ...
    if request.method == "POST":
        # Making sure the username is submitted:
        if not request.form.get("username"):
            return apology("must provide username", 400)

        # Making sure the password is submitted:
        elif not request.form.get("password"):
            return apology("must provide password", 400)

        # Making sure the password confirmation is submitted:
        elif not request.form.get("confirmation"):
            return apology("must provide password confirmation", 400)

        # Making sure the password is the same:
        elif request.form.get("password") != request.form.get("confirmation"):
            return apology("passwords must match", 400)

        # Searching for username in data:
        rows = db.execute(
            "SELECT * FROM users WHERE username = ?", request.form.get("username")
        )
        if len(rows) != 0:
